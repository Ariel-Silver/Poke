...
export default {
  ...
  methods: {
    ...
    getError(entry) {
      ...
      switch (entry) {
        case 'fallo':
          this.error = `No se encontro Pokémon ${this.newPokemon} consultado`;
          break;
        case "empty":
          this.error = 'No ingreso ningun nombre de Pokémon';
          break;
      }
    },
    cleanError() {
    ...
    computed: {
      ...
      abilitiesPokemon: function() {
        if(this.dataPokemon)
          return this.dataPokemon.abilities.map( a => a.ability.name);
        return [];
      }
    }
  }
